...
namespace LiveCaptionTranslator.Models.IBMWatson
{
    public class IBMWatsonSpeechToTextWebsocketService
    {
        ...
        private static string BaseUrlWithModel = "{0}/v1/recognize?access_token={1}&model={2}";
        private static string BaseUrl = "{0}/v1/recognize?access_token={1}";
        ...
        public IBMWatsonSpeechToTextWebsocketService(string url, string token, string model)
        {
            this.URL = url;
        ...
        public async Task StartConnection()
        {
            ...
            if (string.IsNullOrEmpty(Model))
            {
                ...
                url = new Uri(string.Format(BaseUrl, this.URL.Replace("https", "wss"), AccessToken));
            }
            else
            {
                //url = new Uri(string.Format(BaseUrlWithModel, Region, AccessToken, Model));
                url = new Uri(string.Format(BaseUrlWithModel, this.URL.Replace("https", "wss"), AccessToken, Model));
            }
